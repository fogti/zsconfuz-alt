...
static string quoted_wrapper(const string &x) {
  const string qxss = ([&x]() {
    ostringstream ss;
    ss << quoted(x);
    return ss.str();
  ...
  // if any inner char is quoted, return
  if(qxss != ("\"" + x + "\"")) return qxss;
...
int main(int argc, char *argv[]) {
  ...
  cout <<
    ...
    "\n"
    "rule runcmd\n"
    "  command = ( cd .. && zscfz-runcmd $COMMAND ) 5>>$results_file 6>>$dynqueue_file && touch $out\n"
    "  description = $COMMAND\n"
    "\n"
    "rule section\n"
