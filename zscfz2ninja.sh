...
# USAGE: zscfz2ninja < ZSconfuz.txt > zscfz.ninja
...
build \$tmpdir/build.ninja: regenerate \$tmpdir/zscfz.txt
  ...
  while read CMD ARGS; do
    ...
    case "$CMD" in
      ...
      (*)
        # command
        if $IN_PREAMBLE; then
          echo "zscfz2ninja: ERROR: command in preamble is unsupported" 1>&2
          exit 1
        fi
...
cat <<EOF
build all: phony \$tmpdir/build.ninja$(print_steplist)
default all
EOF
)
